    ...
    <div v-for="(task,index) in alltasks.list">
      <group>
        ...
        <flexbox>
          ...
          <flexbox-item>
            <x-button type="warn" @click.native="giveup(task.id)">未完成任务</x-button>
          </flexbox-item>
        </flexbox>
      </group>
  ...
  export default {
    ...
    data() {
      return {
        ...
        myjs: '',
        alltasks: [],
        taskapplyid: this.$route.params.id
      }
    },
    created() {
      this.fetchdata()
    ...
    methods: {
      ...
      getalltasksundo() {
        this.$http.post(localStorage.getItem("url") + "/task/gettaskbyapplyid", {
          applyid: this.taskapplyid,
          pagenum: '1',
          pagesize: '10'
